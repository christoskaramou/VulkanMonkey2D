...
namespace vm {
	...
	void Game1::load()
	{
		...
		rect1 = rect1 * SCALE;
		objects.push_back(Entity(std::make_shared<Sprite>(rect1)));
		...
		for (int i = 0; i < 500 * SCALE; i++) {
			...
			objects.push_back(Entity(std::make_shared<Sprite>(rect1, "textures/cd.png")));
	...
	void Game1::checkInput(double delta)
	{
		if (window.getKey(KEY_A)) {
			window.getRenderer().mainCamera.position.x -= 400.0f * static_cast<float>(delta);
		}
		...
		if (window.getKey(KEY_W)) {
			window.getRenderer().mainCamera.position.y += 400.0f * static_cast<float>(delta);
		}
		...
		if (window.getKey(KEY_SPACE)) {
			if (objects[4].body->GetAngularVelocity() != -1.f)
				objects[4].body->SetAngularVelocity(-1.0f);
		}
		else {
			if (objects[4].body->GetAngularVelocity() != 0.0f)
				objects[4].body->SetAngularVelocity(0.0f);
		}
	...
	{
		vm::Game1* app = reinterpret_cast<vm::Game1*>(glfwGetWindowUserPointer(window));
		float zoom = (float)yoffset / 50.0f;
